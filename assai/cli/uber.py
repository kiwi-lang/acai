...
_ENV_KEY = "_ASSAI_UBER_ARGS"


def create_app():
    """App factory called by uvicorn on each reload.

    Reads the uber CLI args from ``_ASSAI_UBER_ARGS`` env var
    ...
    if not raw:
        raise RuntimeError(
            "create_app() requires the _ASSAI_UBER_ARGS env var. "
            "Run via `assai uber --debug 1`."
        )
    uber = json.loads(raw)

    from assai.cli import setup as _setup
