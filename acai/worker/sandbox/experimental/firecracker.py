...
Requirements:
    ...
    - A guest kernel (vmlinux) and root filesystem (ext4 image) that
      contain Python and the ``acai`` package.  The rootfs should
      have ``acai mcp`` as the init or service process.
...
Architecture::
    ...
    Host                          MicroVM (KVM)
    ┌─────────────┐               ┌──────────────────┐
    │ Orchestrator │─── HTTP ────▶│  acai mcp :9200  │
...
class FirecrackerSandbox(Sandbox):
    ...
    rootfs:
        ...
        contain the ``acai`` package and boot into a state where
        ``acai mcp --host 0.0.0.0 --port 9200`` is running.
    mcp_port:
    ...
    def start(
        ...
        self._workdir = tempfile.mkdtemp(prefix=f"acai-fc-{session_id}-")
